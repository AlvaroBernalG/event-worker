;(function innerSelf(){const _newId=((id=0)=>()=>id+=1)();const _newEventId=eventName=>`${_newId()}_${eventName}_`;const _onMessage=fn=>({data})=>{const[id,eventName,error,payload]=data;fn.call(fn,id,eventName,error,payload)};const _fromFuncToURL=func=>{const codeToInject=`
      ...
      `;return URL.createObjectURL(new Blob([codeToInject],{type:'application/javascript'}))};const _wrapSelfExecFn=(str,params='')=>`;(${str})(${params});`;const _isFunc=test=>test instanceof Function;const _isString=test=>typeof test==='string';const _createResponseBundle=Symbol('_createResponseBundle');const _createRejectBundle=Symbol('_createRejectBundle');const _deleteCallback=Symbol('_deleteCallback');const _onIncomingMessage=Symbol('_onIncomingMessage');class EventWorker{constructor(opts){this.callbacks={};this.terminated=false;// if opts is undefined, I assume that the execution environment is inside a Worker.js
